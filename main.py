...
# ===== УСТАНОВКА НИКА В ГРУППЕ (БЕЗ ПРАВ АДМИНА) =====
async def set_user_nickname(user_id: int, nickname: str):
    try:
        # Устанавливаем имя пользователя в группе (как у админа, но без прав)
...
async def accept_application(callback: CallbackQuery):
    ...
    # Устанавливаем ник в группе (как у админа, но без прав)
