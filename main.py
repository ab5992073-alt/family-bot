...
# Telegram user-account API для чтения личных диалогов владельца.
...
# ID владельца.
# Можно задать через Render Environment → SUPER_ADMIN_ID.
SUPER_ADMIN_RAW = os.environ.get("SUPER_ADMIN_ID", "6166697485").strip()

try:
    SUPER_ADMIN = int(SUPER_ADMIN_RAW)
except ValueError:
    raise RuntimeError(
        "SUPER_ADMIN_ID должен быть числом Telegram user ID."
    )
...
# ВАЖНО:
# Список устанавливается только один раз.
# Если ты потом удалишь зама, он уже не появится снова после перезапуска.
...
async def init_telegram_user_client():
    """
    Подключает пользовательскую Telegram-сессию.
    Она используется для получения личных Telegram-диалогов владельца.
    """

    global telegram_user_client
    global telegram_user_ready

    if TelegramClient is None:
        print(
            "⚠️ Telethon не установлен. "
            "Добавь telethon в requirements.txt"
        )
        return False
    ...
    if not (
        TG_API_ID
        and TG_API_HASH
        and TG_SESSION_STRING
    ):
        print(
            "⚠️ TG_API_ID/TG_API_HASH/TG_SESSION_STRING "
            "не настроены."
        )
        return False
    ...
    try:
        ...
        if not await telegram_user_client.is_user_authorized():
            print(
                "⚠️ TG_SESSION_STRING не авторизован."
            )
        ...
        print(
            "✅ Telegram user-session подключена: "
        ...
        print(
            f"⚠️ Не удалось подключить Telegram user-session: {e}"
        )
...
def telegram_user_is_ready():
    return (
        telegram_user_client is not None
        and telegram_user_ready
    )


async def get_owner_dialog_users(limit=100):
    """
    Возвращает людей из личных диалогов владельца.
    ...
    try:
        async for dialog in telegram_user_client.iter_dialogs(
            ...
            if TgUser is not None and isinstance(
                entity,
                TgUser,
            ):
        ...
        print(
            f"⚠️ Ошибка чтения Telegram-диалогов: {e}"
        )
...
def owner_dialog_page_keyboard(
    users,
    page=0,
    page_size=20,
    prefix="select_admin",
    ...
    for user in chunk:
        label = owner_dialog_short_label(
            user
        )
...
@dp.message(
    lambda m:
    m.from_user.id in user_surveys
)
...
async def show_owner_people_picker(
    message: Message,
    mode: str,
    page: int = 0,
):
    ...
    if not users:
        if not telegram_user_is_ready():
            await message.answer(
                (
                    "⚠️ Не могу получить "
                    "личные Telegram-диалоги владельца.\n\n"
                    "Нужны "
    ...
    if mode == "add_admin":
        ...
        if not users:
            await message.answer(
                (
                    "📭 Среди твоих Telegram-диалогов "
                    "нет привязанных замов.\n"
                    "Удаление по игровому нику доступно "
                    "через /remove_zam Ник"
                )
            )
            return
    ...
    await message.answer(
        ...
        reply_markup=owner_dialog_page_keyboard(
            users,
            page=page,
            prefix=prefix,
        ),
...
async def admin_panel(
    ...
    await message.answer(
        (
            ...
            "Теперь админы назначаются "
            "выбором человека из твоих "
            "личных Telegram-диалогов.\n\n"
            "Используй "
            "<code>/add admin</code> "
            "или кнопку ниже."
        ),
...
async def adm_add_cb(
    ...
    await show_owner_people_picker(
        cb.message,
        "add_admin",
    )
...
async def select_admin_page_cb(
    cb: CallbackQuery
):
    if not is_super_admin(
        cb.from_user.id
...
@dp.callback_query(
    F.data.startswith(
        "remove_admin_user_page:"
    )
)
async def remove_admin_page_cb(
    cb: CallbackQuery
):
    if not is_super_admin(
        cb.from_user.id
...
async def remove_admin_user_cb(
    cb: CallbackQuery
):
    if not is_super_admin(
        cb.from_user.id
...
async def remove_command(
    ...
    if len(args) == 1:
        await message.answer(
            (
                "❌ Использование: "
                "<code>/remove admin</code> "
                "или "
                "<code>/remove zam</code>"
            )
        )
        return
...
# Старые команды оставлены как алиасы.
@dp.message(
    Command("add_admin")
)
async def add_admin_legacy(
    message: Message
):
    if not is_super_admin(
        message.from_user.id
...
async def select_zam_cb(
    ...
    await cb.message.answer(
        (
            f"👑 Выбран: "
            f"<b>{owner_dialog_label(selected)}</b>\n\n"
            "Теперь отправь <b>игровой ник</b> "
            "этого зама одним сообщением.\n"
            "Например: "
            "<code>Sergey_Darknes</code>"
        )
    )
...
@dp.callback_query(
    F.data.startswith(
        "remove_zam_user_page:"
    )
)
async def remove_zam_page_cb(
    cb: CallbackQuery
):
    if not is_super_admin(
        cb.from_user.id
...
@dp.callback_query(
    F.data == "owner_select_cancel"
)
async def owner_select_cancel_cb(
    cb: CallbackQuery
):
    pending_zam_users.pop(
        cb.from_user.id,
        None,
    )
...
@dp.message(
    lambda m:
    m.from_user.id
    in pending_zam_users
)
async def pending_zam_nick_handler(
    message: Message
):
    if not is_super_admin(
        message.from_user.id
    ...
    if not message.text:
        await message.answer(
            "❌ Отправь игровой ник текстом."
        )
        return

    user_id = pending_zam_users.pop(
        message.from_user.id
    )
    ...
    await log_action(
        ...
        (
            f"{game_nick} / "
            f"{owner_dialog_short_label(selected)}"
        ),
    )
...
async def send_zam_stats(
    ...
    text += (
        "ℹ️ Статистика считается "
        "по текущим анкетам.\n"
        "После вывода использованные "
        "приглашения повторно не начисляются."
    )
...
async def myid_cmd(
    ...
    await message.answer(
        (
            ...
            "Для владельца этот ID должен быть "
            "указан в переменной "
...
async def help_cmd(
    ...
    if is_super_admin(
        ...
        text = (
            ...
            "/add_admin — алиас добавления админа\n"
            "/remove_admin — алиас удаления админа\n"
...
async def set_token(
    ...
    await message.answer(
        (
            "🔐 Смена токена отключена "
            "в этой версии.\n"
            "Токен должен храниться только "
            "в Render → Environment → BOT_TOKEN.\n\n"
            "После изменения BOT_TOKEN "
            "в Render перезапусти сервис."
        )
    )
...
OWNER_COMMANDS = ADMIN_COMMANDS + [
    BotCommand(
        command="add",
        description="➕ Выдать админа/зама",
    ),
    BotCommand(
        command="remove",
        description="➖ Убрать админа/зама",
    ),
    BotCommand(
        command="add_admin",
        description="➕ Админ (алиас)",
    ),
    BotCommand(
        command="remove_admin",
        description="➖ Админ (алиас)",
...
async def set_command_scopes():
    # Обычным игрокам — базовые команды.
